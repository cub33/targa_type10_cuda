...
__global__ void writeFrame(AVFrame *cudaFrame, VideoFrame *cudaFrametoExp) {
  printf("HEELLLLLLLOOOOOO");

  //RGBPixel *pixel = (RGBPixel *) malloc(sizeof(RGBPixel)); // eheh
  int idx = blockIdx.x + threadIdx.x * blockDim.x;
  printf("%d, ", idx);
  //int p = threadIdx.x * 3 + blockIdx.x * cudaFrame->linesize[0];
  //cudaFrametoExp->width = blockDim.x;
  //cudaFrametoExp->height = height;
  //cudaFrametoExp->pixels = (RGBPixel *) malloc(sizeof(RGBPixel) * size);
    ...
    // Decode video frame
      ...
      if(frameFinished) {
        ...
        if(once) {
          ...
          AVFrame *cudaFrame;
          VideoFrame *cudaFrameToExp;
          frameToExp = (VideoFrame *) malloc(sizeof(VideoFrame));
          cudaFrameToExp = (VideoFrame *) malloc(sizeof(VideoFrame));
          cudaMalloc((void **) &cudaFrame, sizeof(AVFrame));
          cudaMalloc((void **) &cudaFrameToExp, sizeof(VideoFrame));
          cudaMemcpy(cudaFrame, pFrameRGB, size, cudaMemcpyHostToDevice);
          printf("height: %d, width: %d\n", height, width);
          writeFrame<<<height, width>>>(cudaFrame, cudaFrameToExp);
          //writeFrame(pFrameRGB, width, height); // TODO put size
          cudaMemcpy(frameToExp, cudaFrameToExp, size, cudaMemcpyDeviceToHost);
          cudaFree(cudaFrameToExp);
          cudaFree(cudaFrame);
          //printFrame(frameToExp);
          cudaProfilerStop();
